...
namespace Chess.Game
{
    public class HumanPlayer : Player
    {
        ...
        void TryMakeMove(Coord startSquare, Coord targetSquare)
        {
            ...
            bool wantsBishopPromotion = Input.GetKey(KeyCode.LeftShift);
            bool wantsRookPromotion = Input.GetKey(KeyCode.LeftControl);
            bool wantsQueenPromotion = !wantsBishopPromotion && !wantsKnightPromotion && !wantsRookPromotion;
            ...
            for (int i = 0; i < legalMoves.Count; i++)
            {
                ...
                if (legalMove.StartSquare == startIndex && legalMove.TargetSquare == targetIndex)
                {
                    ...
                    {
                        switch (legalMove.MoveFlag)
                        {
                            case Move.Flag.PromoteToQueen:
                                if (!wantsQueenPromotion)
                                {
                                    continue;
                                }
                                break;
                            case Move.Flag.PromoteToRook:
                                if (!wantsRookPromotion)
                                {
                                    continue;
                                }
                                break;
                            case Move.Flag.PromoteToBishop:
                                if (!wantsBishopPromotion)
                                {
                                    continue;
                                }
                                break;
                            case Move.Flag.PromoteToKnight:
                                if (!wantsKnightPromotion)
                                {
                                    continue;
                                }
                                break;
