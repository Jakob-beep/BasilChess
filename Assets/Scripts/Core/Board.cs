namespace Chess
{
    ...
    public class Board
    {
        ...
        public void UnmakeMove(Move move, bool inSearch = false)
        {
            ...
            bool isEnPassantWest = moveFlags == Move.Flag.EnPassantWest;
            bool isEnPassantEast = moveFlags == Move.Flag.EnPassantEast;
            bool isPromotion = move.IsPromotion;
            ...
            if (capturedPieceType != 0 && !(isEnPassantWest || isEnPassantEast))
            {
            ...
            else if (isEnPassantWest)
            { // ep cature: put captured pawn back on right square
                int epIndex = movedTo + ((ColourToMove == Piece.White) ? -9 :7);
                Square[movedTo] = 0;
            ...
            else if (isEnPassantEast)
            { // ep cature: put captured pawn back on right square
                int epIndex = movedTo + ((ColourToMove == Piece.White) ? -7 : 9);
                Square[movedTo] = 0;
                Square[epIndex] = (int)capturedPiece;
                pawns[opponentColourIndex].AddPieceAtSquare(epIndex);
                ZobristKey ^= Zobrist.piecesArray[Piece.Pawn, opponentColourIndex, epIndex];
